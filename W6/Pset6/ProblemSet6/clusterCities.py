...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        # 1. define mindistance
        minDistance = 100000.0

        # for all points in this cluster
        for point in self.points:
            for Opoint in other.points:
                dist = float(point.distance(Opoint))
                if dist < minDistance:
                    minDistance = dist
    ...
    def averageLinkageDist(self, other):
        ...
        avg = float(sum(distList) / len(distList))
        return float(avg)
        pass
...
class ClusterSet(object):
    ...
    def findClosest(self, linkage):
        ...
        if linkage == Cluster.singleLinkageDist:
            for clust in self.members:
                for clust2 in self.members:
                    if clust != clust2:
                        if bestDist > clust.singleLinkageDist(clust2):
                            bestDist = clust.singleLinkageDist(clust2)
                            bestClust1 = clust
                            bestClust2 = clust2

        if linkage == Cluster.maxLinkageDist:
            for clust in self.members:
                for clust2 in self.members:
                    if clust != clust2:
                        if bestDist > clust.maxLinkageDist(clust2):
                            bestDist = clust.maxLinkageDist(clust2)
                            bestClust1 = clust
                            bestClust2 = clust2

        if linkage == Cluster.averageLinkageDist:
            for clust in self.members:
                for clust2 in self.members:
                    if clust != clust2:
                        if bestDist > clust.averageLinkageDist(clust2):
                            bestDist = clust.averageLinkageDist(clust2)
                            bestClust1 = clust
                            bestClust2 = clust2

        return bestClust1, bestClust2
        pass
    ...
    def mergeOne(self, linkage):
        ...
        clust1, clust2 = self.findClosest(linkage)

        self.mergeClusters(clust1, clust2)
