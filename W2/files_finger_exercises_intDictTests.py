...
def min_class_size(top_size, low_size):
    """

    :param top_size: upper bound number of the likely range of class sizes
    :param low_size: lower bound number of the likely range of class sizes
    :return: minimum class size necessary to have less than 0.99 probability of 2 students sharing the same birthday
    """
    goal_prob = 0.99
    mid_size = (top_size + low_size)/2
    prob = collision_prob(365, mid_size)
    pdb.set_trace()
    if prob > goal_prob:
        # probability is still too high, reduce class size
        new_top = mid_size
        return min_class_size(new_top, low_size)
    elif prob < goal_prob:
        # probability too low, increase class size
        new_low = mid_size
        return min_class_size(top_size, new_low)
    else:
        # return probability for 0.98
        return mid_size